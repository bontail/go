...
func encodeBlock(x archsimd.Uint8x16) (encodedLo, encodedHi archsimd.Uint8x16) {
	...
	// Pack each digit pair into a uint16 lane as highDigit | lowDigit<<8.
	// Wasm's little-endian byte order places highDigit first in memory.
...
func decodeBlock(input archsimd.Uint8x16, dst []byte) (invalid bool) {
	...
	storeUint64LE(dst, packed.GetElem(0))
	return false
}
